...
var utils = function () {
   ...
   */
		...
		value: function matchUserAgent(str) {
			var navigator = globals.window.navigator;
...
var App$1 = function (_EventEmitter) {
  ...
  */
   ...
   */
		...
		_this.appEventHandlers_.add(on(globals.window, 'scroll', utils.debounce(_this.onScroll_.bind(_this), 100)), on(globals.window, 'load', _this.onLoad_.bind(_this)), on(globals.window, 'popstate', _this.onPopstate_.bind(_this)));
   ...
   */
		...
		value: function load(path) {
			...
			return Promise.race([fetch(request).then(function (resp) {
				...
				setTimeout(function () {
					return reject(new Error(errors.REQUEST_TIMEOUT));
				}, _this2.timeout);
			})]).catch(function (reason) {
				_this2.removeSafariXHRPolyfill();
  ...
  */
   ...
   */
		...
		value: function runFaviconInElement_(elements) {
			return new Promise(function (resolve) {
				elements.forEach(function (element) {
					return document.head.appendChild(utils.isIe() ? element : utils.setElementWithRandomHref(element));
				});
				resolve();
			});
		}
